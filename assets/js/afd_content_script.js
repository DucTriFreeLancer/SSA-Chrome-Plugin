...
var ADF_memberListSelectorNew =  'div.i1n1lj7b.mmwt03ec div[data-visualcompletion="ignore-dynamic"]:not(.adf-processed)';
		...
		chrome.storage.local.get(["ADF_groupSettings","linkedFbAccount"], function(result) {
		    if (typeof result.ADF_groupSettings != "undefined") {
		    		...
		    		if ($('#bluebarRoot').length == 0) { /// new layout 
						totalGroupMembers = $('h2.jxuftiz4.jwegzro5.hl4rid49.icdlwmnq.gvxzyvdx.aeinzg81:contains(Members):eq(0)').text();
...
function AFD_processGroupMembersForNew(history = 0) {	
	...
	ADF_total = $('h2.jxuftiz4.jwegzro5.hl4rid49.icdlwmnq.gvxzyvdx.aeinzg81:contains(Members):eq(0)').text().replace(/[^\d]/g, '');
						...
						timeOutIds = setTimeout(()=>{
							if (ADF_add_friend_totalSend <= ADF_limit-1 && ADF_add_friend_processing && !ADF_add_friend_stopProcess) {
								$(this).addClass('adf-processed');
									...
									var description = $(this).find('.jroqu855.nthtkgg5:eq(2) span').text().toLowerCase();
								...
								if (validDescription) {
									...
									if(btnText.length == 1){
										$('.member-name').text('Sending to '+ $(this).find('.rse6dlih a').text()).css('text-align','center');
